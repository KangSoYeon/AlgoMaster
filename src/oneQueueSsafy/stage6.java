...
public class stage6 {
	...
	static final String HOST = "70.12.108.127";
			...
			while (socket != null) {
				...
				for(int i=1; i<=4; i++) {
					...
					if((balls[0][0]==65 && balls[0][1] == 65)
							&& (balls[1][0] == 195 && balls[1][1]==65)) { //처음에 뿌시는거 
						angle = 91;
						power = 180;
						break;
					}
	...
	private static void shoot(int[] go, int[] target, double i) {
		angle = getDegree(go, target, i);
	...
	private static double getDegree(int[] go, int[] target, double idx) {
		//평행이동 해서 각도값 return
		double mx = go[0];
		double my = go[1];
		double mtx = target[0] - mx; //평행이동
